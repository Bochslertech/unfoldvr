        ...
        switch (v.get("mimeType"),v.get("url")) {
            case (?#String(mimeType),?#String(url)) {
              if(mimeType == "video"){
                ...
                let PREFIX = "<html><head><meta name='viewport'content='width=device-width, initial-scale=1.0'/></head><body><script type='module'src='https://unpkg.com/@google/model-viewer@1.12.0/dist/model-viewer.min.js'></script><model-viewer id='model'style='width: 100%;height: 100%;'src='";
                let POSTFIX = "'camera-controls autoplay ar ar-status='not-presenting'/></body></html>";
                let padd = Blob.toArray(Text.encodeUtf8(PREFIX));
                let metaArr = Blob.toArray(Text.encodeUtf8(url));
                let suff = Blob.toArray(Text.encodeUtf8(POSTFIX));
                return ?Blob.fromArray(Array.append(Array.append(padd, metaArr), suff));
